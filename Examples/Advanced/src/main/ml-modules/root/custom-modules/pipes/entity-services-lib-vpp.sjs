...
function validateAndRunMappingByDoc(mapping, doc) {
  let validatedMapping = validateMapping(mapping);
  return runMappingByDoc(validatedMapping, doc);
}
/**
...
function runMappingByDoc(mapping, doc, propMapping={"targetEntityType":mapping.targetEntityType,"properties": {}}, paths=['properties']) {
  Object.keys(mapping.properties).forEach(propertyName => {
    let mappedProperty = mapping.properties[propertyName];
    let sourcedFrom = mappedProperty.sourcedFrom;
    ...
    if(!mappedProperty.errorMessage){
      if (mappedProperty.hasOwnProperty("targetEntityType")) {
        ...
        mappedProperty = runMappingByDoc(mappedProperty, doc, propMapping, paths);
        paths.pop();
      }
      else {
    ...
    if(mappedProperty && !mappedProperty.errorMessage && ! mappedProperty.hasOwnProperty("targetEntityType")){
      let resp = getCanonicalInstanceByDoc(propMapping, doc, propertyName);
      if(resp && resp.output) {
...
function getCanonicalInstanceByDoc(mapping, doc, propertyName) {
  let resp = {};
  let xmlMapping = buildMappingXML(fn.head(xdmp.unquote(xdmp.quote(mapping))));
  //let doc = cts.doc(uri);
