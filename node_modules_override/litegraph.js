(function (global) {
  ...
  var LiteGraph = (global.LiteGraph = {
    ...
    // PIPES BEGIN
    // Signal entry/exit from subgraphs back
    pipesCallbackItem: null,
    registerPipesListener: function (component) {
      this.pipesCallbackItem = component
    },

    signalEnteringSubgraph: function (title) {
      if (this.pipesCallbackItem != null)
        this.pipesCallbackItem.$root.$emit("enteredSubGraph", title)
    },
    signalExitingSubgraph: function () {
      if (this.pipesCallbackItem != null)
        this.pipesCallbackItem.$root.$emit("exitedSubGraph")
    },
    // block details to Pipes for display (description in popup)
    sendblockDetails: function(b) {
      if (this.pipesCallbackItem != null)
        this.pipesCallbackItem.$root.$emit("blockDetails",b)
    },
    closeblockDetails: function() {
      if (this.pipesCallbackItem != null)
        this.pipesCallbackItem.$root.$emit("closeBlockDetails")
    },
    setStyling: function (styles) {
      //const props = ['NODE_TITLE_COLOR','NODE_TEXT_SIZE','NODE_TEXT_COLOR','NODE_SUBTEXT_SIZE','NODE_DEFAULT_COLOR','NODE_DEFAULT_BGCOLOR','NODE_DEFAULT_BOXCOLOR',]
      if (styles !== null) {
        for (var prop in styles) {
          if (Object.prototype.hasOwnProperty.call(styles, prop)) {
            console.log("Setting " + prop + " in litegraph to " + styles[prop])
            LiteGraph[prop] = styles[prop]
          }
        }
      }
    },
    getRegisteredNodes: function () {
      var blocks=[]
      Object.getOwnPropertyNames(this.registered_node_types).forEach(key => {
        blocks.push(key)
      });
      return blocks
    },
    // remove a node from Litegraph
    deregisterNode: function (nodeKey) {
      delete this.registered_node_types[nodeKey]
    },
    // PIPES END
...
*/
  ...
  LGraphCanvas.prototype.closeSubgraph = function () {
    ...
    // PIPES BEGIN
    LiteGraph.signalExitingSubgraph()
    // PIPES END
  ...
  LGraphCanvas.prototype.bindEvents = function () {
    ...
    document.addEventListener("keydown", this._key_callback, true);  // Fix ERP
    ...
    if (
      ...
      if (this.render_connection_arrows) {
        ...
        if (this.render_curved_connections) {
          ...
          angleB = angleA = b[1] > a[1] ? 0 : -Math.PI / 2;
        }
    ...
    for (var i = 0; i < widgets.length; ++i) {
      ...
      switch (w.type) {
        ...
        case "text":
          ...
          if (show_text) {
            ctx.fillStyle = secondary_text_color;
            // PIPES TICKET #121 begin
            // if (w.name != null) {
            if (w.name != null && w.value.length == 0) { // messy - no longer show type if also value to display
              ctx.fillText(w.name, margin * 2, y + H * 0.7);
            }
            ctx.fillStyle = text_color;
            ctx.textAlign = "right";
            // main logic to clip text
            var displayTxt = w.value
            if (ctx.measureText(displayTxt).width > (width - margin * 3)) {
              // build up string until it no longer fits
              for (var c = 1; c < w.value.length; c++) {
                displayTxt = w.value.substring(0, c)
                var metrics = ctx.measureText(displayTxt).width;
                if (metrics > (width - margin * 3.5)) {
                  displayTxt = w.value.substring(0, c - 1) + ".."
                  break;
                }
              }
            }
            ctx.fillText(displayTxt, width - margin * 2, y + H * 0.7);
  ...
  LGraphCanvas.onMenuAdd = function (node, options, e, prev_menu) {
    ...
    function inner_create (v, e) {
      console.log("inner creating block..")
  ...
  function ContextMenu (values, options) {
    ...
    root.addEventListener("mouseleave", function (e) {
      ...
      // PIPES
      root.closing_timer = setTimeout(function() {LiteGraph.closeblockDetails(); that.close.bind(that, e);} , 500);
  ...
  ContextMenu.prototype.addItem = function (name, value, options) {
    ...
    // PIPES block details
      ...
      element.addEventListener("mouseenter", block_description_popup);
      element.addEventListener("mouseout", block_description_hide);
    }
    function block_description_hide(e) {
      LiteGraph.closeblockDetails()
    }
    function block_description_popup(e) {
      var value = this.value;
      var desc = ''
      var title = ''
      if (value.value && value.value !== null && value.value != undefined) {
        try {
          var BLOCKKEY = value.value
          if ( LiteGraph.registered_node_types[BLOCKKEY] !== null && LiteGraph.registered_node_types[BLOCKKEY] != undefined  ) {
            desc = LiteGraph.registered_node_types[BLOCKKEY].description
            title = LiteGraph.registered_node_types[BLOCKKEY].title
          } else {
            //console.log("Block cound't be found.. " )
          }
      ...
      LiteGraph.sendblockDetails({"block" : value.value, "title" : title, "description" : desc})
    }
    // PIPES ENDX
    ...
    function inner_onclick (e) {
      LiteGraph.closeblockDetails(); // PIPES - CLOSE BLOCK DESCRIPTION POPUP
      var value = this.value;
  ...
  Subgraph.description = "Adds a node containing a further graph";
  Subgraph.title_color = "#334";


  Subgraph.prototype.onConfigure = function (config) {
    this.isStarted=true
    let inLength = this.inputs.length
    let outLength = this.outputs.length
    ...
    if (inLength > 0)
      for (let j = 0; j < inLength; j++) {
        let name = this.inputs[j - dec].name
        if (inputList.has(this.inputs[j - dec].name))
          this.removeInput(j - dec++)
        //inToDelete.push(j)
        inputList.add(name)
        console.log(name)
      }
    ...
    if (outLength > 0)
      for (let j = 0; j < outLength; j++) {
        let name = this.outputs[j - dec].name
        if (outputList.has(this.outputs[j - dec].name))
          this.removeOutput(j - dec++)
        //outToDelete.push(j)
        outputList.add(name)
      }
  ...
  Subgraph.prototype.onDblClick = function (e, pos, graphcanvas) {
    var that = this;
    // PIPES BEGIN
    LiteGraph.signalEnteringSubgraph(this.title)
    // PIPES END
    setTimeout(function () {
    ...
    return [
      {
        ...
        callback: function () {
          // PIPES BEGIN
          LiteGraph.signalEnteringSubgraph(that.title)
          // PIPES END
          graphcanvas.openSubgraph(that.subgraph);
        }
      }
  ...
  Subgraph.prototype.onDrawForeground = function () {
    /*
     if (this.inputs && this.inputs.length>0) {
       let slot = this.inputs[0];

       if(slot.name =="item from item(s)")
         slot.label = "item(s)" //slot.name + " [Array]"
     }
     if (this.outputs && this.outputs.length>0 ) {
       let slot = this.outputs[0];

       if(slot.name =="result item")
             slot.label = "result item(s)" //slot.name + " [Array]"
     }
     */
  };
  ...
  function GraphInput () {
    ...
    Object.defineProperty(this.properties, "name", {
      ...
      set: function (v) {
        ...
        if (!that.properties.isMain) {
          if (that.name_in_graph) {
            //already added
            that.graph.renameInput(that.name_in_graph, v);
          } else {
            if(! this.isStarted) that.graph.addInput(v, null);
          }
          ...
          if (that.name_in_graph) {
            //already added
            that.graph.renameInput(that.name_in_graph, v + " [List]");

          }
          else {
            if(! this.isStarted) that.graph.addInput(v + " [List]", null);

          }
          that.name_widget.value = v;
          that.name_in_graph = v + " [List]";
        }
      },
      enumerable: true
    });
  ...
  GraphInput.description = "Creates the input for a subgraph";

  GraphInput.prototype.onConfigure = function (config) {
    console.log("isStarted")
    this.isStarted=true
  }
  GraphInput.prototype.onAdded = function () {
    if (this.properties.name == "") {

      if (this.properties.isMain) {
        randomName = "item"
      } else
        randomName = "RenameInput_" + String(Math.round(Math.random() * 100)).padStart(3, '0')
      this.properties.name = randomName;
    }
  }
      ...
      set: function (v) {
        ...
        if (!that.properties.isMain) {
          if (that.name_in_graph) {
            ...
            if(! this.isStarted) that.graph.addOutput(v, null);
          }
          that.name_widget.value = v;
          that.name_in_graph = v;
        }
          ...
          if (that.name_in_graph) {
            //already added
            that.graph.renameOutput(that.name_in_graph, v + " [List]");
            that.name_in_graph = v + " [List]";
          }
          else {
            if(! this.isStarted) that.graph.addOutput(v + " [List]", null);
            that.name_in_graph = v + " [List]";
          }
          that.name_widget.value = v;
        }
    ...
    /*setTimeout(function(){
       if(this.properties.name==""){
         ...
         if(this.isMain) {
           ...
           randomName ="RenameOutput_" +String(Math.round(Math.random() *100)).padStart(3,'0')
         this.properties.name = randomName;
  ...
  GraphOutput.description = "Creates the output for a subgraph";

  GraphOutput.prototype.onExecute = function () {
    this._value = this.getInputData(0);
    // this.graph.setOutputData(this.properties.name, this._value);
    this.graph.setOutputData("result items", this._value);
  };

  GraphOutput.prototype.onAdded = function () {
...
(function (global) {
  ...
  GraphInputDHF.title = "Custom Step Input";
  GraphInputDHF.description = "Entry point from DHF into custom step graph";
  ...
  GraphOutputObjectDHF.title = "Envelope";
  GraphOutputObjectDHF.description = "Wraps document in an envelope (DHF Envelope pattern)";
  ...
  GraphOutputDHF.title = "Custom Step Output";
  GraphOutputDHF.description = "Exit point from custom step graph back into DHF";
...
(function (global) {
  ...
  currentDate.description = "Outputs the current date or datetime";
